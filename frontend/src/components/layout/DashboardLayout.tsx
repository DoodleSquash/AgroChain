...
export default function DashboardLayout() {
  ...
  const FARMER_NAV = [
    ...
    { label: t('nav.payments'), path: '/farmer/payments', icon: 'account_balance_wallet' },
  ];

  const MARKET_NAV = [
    { label: t('nav.marketplace'), path: '/market/browse', icon: 'shopping_cart' },
    { label: t('nav.my_orders'), path: '/market/dashboard', icon: 'receipt_long' },
    { label: t('nav.logistics'), path: '/market/logistics', icon: 'local_shipping' },
  ];

  const navItems = isFarmer ? FARMER_NAV : MARKET_NAV;
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const userName = user.name || (isFarmer ? t('nav.farmer_portal') : t('nav.buyer_portal'));
      ...
      <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-2xl shadow-sm flex justify-between items-center px-3 sm:px-4 md:px-6 py-2 border-b border-outline-variant/20 text-on-surface transition-all">
        ...
        <div className="flex items-center gap-2 md:gap-5">
          ...
          <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-primary-200 bg-primary-50 text-primary-700">
             ...
             <span className="text-[11px] font-bold uppercase tracking-wider">{isFarmer ? t('nav.farmer_portal') : t('nav.buyer_portal')}</span>
          </div>
          ...
          <button className="relative p-2 hover:bg-surface-container rounded-full transition-colors text-on-surface-variant group">
            <span className="material-symbols-outlined group-hover:rotate-12 transition-transform text-[22px] sm:text-[24px]">notifications</span>
          ...
          <div className="flex items-center gap-2 cursor-pointer hover:bg-surface-container-low p-1 rounded-full transition-colors border border-transparent hover:border-outline-variant/20 group">
             ...
             <div className="hidden md:flex flex-col">
               ...
               <span className="text-[10px] text-primary-600 font-bold flex items-center gap-0.5">
                  <span className="material-symbols-outlined text-[12px]" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span> {t('dashboard.verified')}
               </span>
             </div>
        ...
        <aside className="hidden lg:flex flex-col w-72 bg-white p-6 sticky top-16 h-[calc(100vh-64px)] border-r border-zinc-200/50 shadow-sm">
          ...
          <nav className="flex-1 flex flex-col gap-1 mt-4">
            <div className="px-3 mb-3">
              <span className="text-[11px] font-black tracking-[0.15em] text-zinc-400 uppercase">{t('nav.general')}</span>
            </div>
                ...
                <React.Fragment key={item.path}>
                  {needsSeparator && (
                    <div className="px-3 mt-6 mb-3">
                      <span className="text-[11px] font-black tracking-[0.15em] text-zinc-400 uppercase">{t('nav.management')}</span>
                    </div>
                  )}
                  <Link
                    to={item.path}
          ...
          <div className="mt-auto pt-6 border-t border-zinc-100 space-y-4">
            <div className="px-3 mb-1">
              <span className="text-[11px] font-black tracking-[0.15em] text-zinc-400 uppercase">{t('nav.account')}</span>
            </div>
